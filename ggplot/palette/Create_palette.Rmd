...
We will follow the steps explained in this tutorial <https://drsimonj.svbtle.com/creating-corporate-colour-palettes-for-ggplot2>. We will need to follow these steps:
...
Now we create a list of combination of the colours defined in 1.
...
This function gets a palete by name from the list ("eurostat" by default), has a boolean condition determining whether to reverse the order or not, and additional arguments to pass on to colorRampPallete() (such as an alpha value).

```{r}
luis_pal("eurostat")
```

This returned function will interpolate the palette colors for a certain number of levels, making it possible to create shades between our original colors. To demonstrate, we can interpolate the "luis" palette (which only includes five colours to a length of 12:

```{r}
scales::show_col(luis_pal("luis")(12))
```
...
ggplot(df, aes(time,obs_value, colour = geo))+
  geom_line(size=1)+
  theme_minimal()+
  scale_colour_luis(palette = "luis", reverse = TRUE)
```
...
ggplot(df %>% filter(time =="2019"), aes(geo,obs_value, fill = geo))+
  geom_col()+
  theme_minimal()+
  scale_fill_luis(palette="eurostat")
```
